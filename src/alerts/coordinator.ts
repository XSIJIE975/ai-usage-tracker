...
export interface AlertCoordinatorDeps {
  ...
  onActiveChange: (providerId: string, active: boolean) => void;
}

/**
 * 告警协调器：维护每个供应商规则的边沿触发与冷却状态。
  ...
  observe(providerId: string, snapshot: ProviderSnapshot, settings: AppSettings): void {
    ...
    if (!settings.alertsEnabled) {
      if (state.triggered) {
        ...
        this.deps.onActiveChange(providerId, false);
      }
      return;
    }

    const fire = evaluateRule(providerId, snapshot, settings.alertThresholds);
    if (fire) {
      ...
      if (!state.triggered) {
        ...
        this.state.set(key, state);
        this.deps.onActiveChange(providerId, true);
      }
      return;
    }

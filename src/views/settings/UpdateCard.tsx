    ...
    <Card>
      <CardHeader className="flex-row items-start justify-between space-y-0">
        ...
        <Button
          ...
          onClick={() => void check()}
          aria-label={status === "checking" ? "检查中" : "检查更新"}
        >
      ...
      <CardContent className="space-y-3">
        {!supported && (
          <p className="text-[13px] text-fg-muted">开发构建不支持检查更新，安装正式包后可用。</p>
        )}
        ...
        {status === "up-to-date" && (
          <p className="text-[13px] text-success">已是最新版本。</p>
        )}
          ...
          <div className="space-y-3 rounded-md border border-brand/30 bg-surface-2 p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button size="sm" onClick={() => void downloadAndInstall()}>
                <Download className="h-3.5 w-3.5" /> 下载并安装
              </Button>
            </div>
            {notes && (
        ...
        {status === "ready" && (
          <p className="text-[13px] text-success">下载完成，即将安装…</p>
        )}

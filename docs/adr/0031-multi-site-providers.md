...
- **国际站凭据形态另开一条（只贴 Cookie 值）**：若实测国际站网关不校验 UA 三元组，粘贴体验更轻。但形态一分叉，弹窗文案、白名单校验、诊断按钮、测试矩阵就变成 2×2；而当时的 cURL 解析器本就兼容只贴 Cookie 头与裸 session 值的降级形态，贴 cURL 在宽松网关下同样成功（这条兼容理由已随解析器退役，形态统一本身仍是决定性的）。统一形态的代价只是多贴一点东西。

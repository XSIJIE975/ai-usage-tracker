...
- **真机验证项（唯一实质风险）**：CodexBar 的可行证据来自 macOS 端；中国站站 Baxia 风控对 Windows + 不同 IP 段 + 写死 UA 的组合是否照单全收未验证。403 的出路是把凭据槽升级为 curl_paste 形态（带真实 UA），凭据库结构支持换槽不改架构。

...
- **粘贴解析器用通用 curl 语法库（`curl-parser` 0.6.0，已实测出局）**：它的 pest 语法是**选项白名单**（`-X/-H/-d/-u/-k/-L` + URL），DevTools 复制必带的 `--compressed`、POST 的 `--data-raw`、cmd 版的 `-b` 与 `^` 续行全都命中不了 → 真实粘贴成功率 0%；且 `-H 'garbage'`（无冒号）会走 `expect` **panic**，在 Tauri command 里就是任务崩溃。故最终用 `shell-words`（零依赖，只负责 POSIX 引号/转义分词）+ 本模块 ~60 行已知键抽取：未知选项跳过而不是报错，恰好是「用户粘贴什么都可能」这个场景需要的性质。踩过的两个坑已锁进单测：cmd 的 `^` 续行必须先把 CR/CRLF 统一成 LF 再按行合并、**合并处必须补空格**（不补则 `-H` 与前一行引号粘成一个 token，后面的头静默全丢，症状与本次 401 一样查不出来）；非 ASCII 粘贴会踩 `text[..7]` 字节边界 panic，前缀比较一律走 `get(..7)`。
...
- 用户需**每登录会话重贴一次 Copy as cURL**（服务端会话有效期未知，可能远短于最初假设的 60 天；换浏览器、换机器、浏览器升版也算）。401 有两种成因——会话失效与三元组不同源（漏 session_2、UA 与登录时不一致），出路同为重贴，故快照文案统一为「登录凭据无效或已过期，请在设置中重新粘贴 Copy as cURL」，不试图区分（区分需要额外一次探测且无法给出不同建议）。期间该实例快照持续报错直到重贴——错误可见（ADR-0024），不会静默用旧数据。
- 凭据槽存的是整段 curl 文本（含 Cookie 与 UA，约 8KB），仍在系统凭据库内加密、不出本机；体积远小于钥匙串上限，但解析入口保留了 256KB 粘贴上限以挡住误粘的大文本。
